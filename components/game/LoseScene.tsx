"use client";

/**
 * LoseScene component - sympathetic visuals for lose screen
 * Matches single-shot design with large emoji and minimal text
 */

import { PlayerCharacterSad } from "./characters/PlayerCharacterSad";
import { Difficulty, DIFFICULTY_OPTIONS } from "@/lib/types";

interface LoseSceneProps {
  destination: string;
  stationsStanding: number;
  totalStations: number;
  difficulty: Difficulty;
}

export function LoseScene({
  destination,
  stationsStanding,
  totalStations,
  difficulty,
}: LoseSceneProps) {
  const difficultyOption = DIFFICULTY_OPTIONS.find((o) => o.value === difficulty);
  const wasClose = stationsStanding >= totalStations - 1;

  return (
    <div className="text-center" data-testid="lose-scene">
      {/* Sad emoji */}
      <div className="text-7xl mb-4">😔</div>

      <h1
        id="game-end-title"
        className="text-3xl font-bold text-rose-600 mb-2"
        data-testid="game-end-title"
      >
        No Seat Found
      </h1>

      <p className="text-stone-600 mb-4" data-testid="game-end-message">
        You arrived at {destination} standing the whole way!
        {wasClose && <span className="block text-sm text-amber-600 mt-1">So close!</span>}
      </p>

      {/* Character and stats */}
      <div className="flex items-center justify-center gap-4 bg-rose-50 rounded-xl p-4">
        <div className="w-16 h-20" data-testid="lose-character">
          <PlayerCharacterSad />
        </div>
        <div className="text-left">
          <p className="text-sm text-stone-600">
            Stood for <span className="font-bold text-rose-600">{stationsStanding}</span> station
            {stationsStanding !== 1 ? "s" : ""}
          </p>
          <p className="text-sm text-stone-500">
            {difficultyOption?.emoji} {difficultyOption?.label} mode
          </p>
        </div>
      </div>
    </div>
  );
}
