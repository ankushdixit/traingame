# CLAUDE.md - Project Guidelines for traingame

## Project Configuration

- **Stack**: Full-Stack Product (Next.js)
- **Quality Tier**: Comprehensive
...
2. **Database access via Prisma**
   ...
   - Database queries can run directly in Server Components
...
3. **Prefer Server Actions over API Routes**
   ...
   - Use API Routes only when:
     ...
     - Server Actions won't work for your use case
...
| ---------------- | -------------------------------- |
| Pages            | `app/`                           |
| API routes       | `app/api/` (only when needed)    |
| Server Actions   | `app/actions/` or colocated      |
| React components | `components/`                    |
| Database schema  | `prisma/schema.prisma`           |
| Utilities        | `lib/`                           |
| Zod schemas      | `lib/validations.ts` (create it) |
...
- Creating API routes for operations that can be Server Actions
...
3. **Follow documented patterns**
   - Reference ARCHITECTURE.md for stack-specific patterns before writing new code
   - Maintain consistency with the patterns documented there
   - This is a minimal scaffolding project - you'll create files from scratch
...
- The user's request conflicts with documented patterns in ARCHITECTURE.md
...
6. **Don't guess command options**
   - NEVER assume you know all options for a command
   - ALWAYS run `sk <command> --help` in terminal if unsure about CLI options
...
# Manual checks for fullstack_nextjs:
npm test                 # Run tests
